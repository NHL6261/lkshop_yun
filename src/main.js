...
// 使用vant组件
Vue.use(DatetimePicker).use(Cell).use(CellGroup).use(Popup).use(Icon).use(Lazyload, {
        lazyComponent: true,
        preLoad: 1.1,
    ...
    .use(SwipeCell).use(Switch).use(PullRefresh).use(List).use(Empty).use(Sticky).use(CountDown).use(ActionSheet).use(Circle).use(Card).use(SubmitBar).use(ShareSheet).use(Sku).use(Area).use(Tag);
// Vue.use(MINT);
// Vue.use(Vuex)
Vue.use(VueRouter);
// Vue.use(ydui);
