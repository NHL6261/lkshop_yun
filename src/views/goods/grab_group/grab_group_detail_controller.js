...
export default {
  ...
  methods: {
    ...
    getData() {
      let that = this;
      $http.get("plugin.snatch-regiment.api.index.getSnatchData", { art: 'raidDetail', leader_id: that.id }, "")
        .then(
          response => {
    ...
    checkActivity() {
      let that = this;
      $http.get("plugin.snatch-regiment.api.index.getSnatchData", { art: 'inspectOverrun', at_id: this.$route.params.id, option_id: this.optionsId, total: this.goodsCount, goods_id: this.info.goods_id }, '')
        .then(
          response => {
            if (response.result === 1) {
    ...
    share(data) {
      let that = this;
      let _titles, _imgUrls, _descs = null;
      try {
        _titles = that.info.title;
        _imgUrls = that.info.thumb;
        _descs = '向您分享一个抢团活动';
      } catch (error) {
        console.log(error);
      }
