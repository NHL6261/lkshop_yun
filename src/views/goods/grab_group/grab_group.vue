  ...
  <div id="grab_group">
    <div class="content" v-show="showGood" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
          ...
          <div class="title_number">
            ...
            <div class="title_Main">
              <h1 style="-webkit-box-orient: vertical;">
                ...
                }}<font class="market-price" v-if="Number(gooddatas.market_price) > 0"
                  >{{ fun.getMarketPrice() }}: {{ gooddatas.market_price }}</font
                >
              </h1>
                  ...
                  <div class="into_user">
                    ...
                    <ul class="time">
                      ...
                      <li class="time_b">
                        <div>
                          剩余<span>
                            <van-count-down :time="fun.getTimeDifference(item.end_time)" format="DD天HH时mm分ss秒" @finish="hideIdexbb"></van-count-down>
                          </span>
                        </div>
                        <div>{{ gooddatas.limit_num }}人团</div>
                      </li>
                    </ul>
                    <button @click="clickAdd(item.leader_id)" v-if="item.is_oneself == 0" type="button">去参团</button>
                    <button @click="clickAdd(item.leader_id)" v-else type="button">邀请好友</button>
                  </div>
                </van-swipe-item>
              </van-swipe>
            </div>
        ...
        <div class="open_group">
          ...
          <div class="setMaxH">
            <div class="into_bg_color" v-if="!fun.isTextEmpty(team) && show2">
              <div class="into_user" v-for="(item, index) in team" :key="index" @click.stop="clickAdd(item.leader_id)">
                <div class="user">
                  <img :src="item.avatar" />
                </div>
                <ul class="time">
                ...
                <button @click.stop="clickAdd(item.leader_id)" v-if="item.is_self == 0" type="button">加入该团</button>
                <button @click.stop="clickAdd(item.leader_id)" v-else type="button">邀请好友</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <template v-if="popupSpecs">
        <yz-specs :goodsInfo="gooddatas" :popupSpecs="popupSpecs" v-on:closeSpecsPopup="close_yz_specs_popup">
