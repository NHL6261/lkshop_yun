...
export default {
  ...
  methods: {
    ...
    checkActivity() {
      let that = this;
      $http.get("plugin.snatch-regiment.api.index.getSnatchData", { art: 'inspectOverrun', at_id: this.gooddatas.activity_id, option_id: this.optionsId, goods_id: this.$route.params.id, total: this.goodsCount, }, "")
        .then(
          response => {
            if (response.result === 1) {
    ...
    setDataByTabIndex() {
      if (this.gooddatas.content) {
        let str = this.gooddatas.content.replace(/src=\"http:/gi,
          "src=\"https:");
        let reg1 = /<iframe[^>]*>(.|\n)*<\/iframe>/gi;
    ...
    OpenLeader() {
      let that = this;
      $http.get("plugin.snatch-regiment.api.team.OpenLeader", { at_id: this.gooddatas.activity_id, goods_id: that.goods_id }).then(response => {
        if (response.result === 1) {
          Toast('开团成功！');
    ...
    postShow() {
      ...
      $http
        .get(
          "plugin.snatch-regiment.api.GoodsPoster.generateGoodsPoster",
          { id: this.goods_id },
          "加载中"
        )
        .then(response => {
    ...
    appShare(data) {
      ...
      let _imgUrl = that.gooddatas.thumb[0];
    ...
    share(data) {
      ...
      wx.ready(function () {
        ...
        let _imgUrls = that.gooddatas.thumb[0];
        let _descs = "邀请您加入抢团活动";
  ...
  components: { Swipe, SwipeItem, cMyswipe, cComment, cCommentlist, yzSpecs },
  watch: {
    scroll() {
      this.loadSroll();
    }
