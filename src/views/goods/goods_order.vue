<template>
  <div id="goodsinfo" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
    ...
    <!-- 配送方式 start -->
    <template v-if="dispatch.length > 0">
    ...
    <template v-if=" is_cps && cpsType == '1' ">
      <div style="background: #fff; height: 3rem;">
         <input type="text" :name="cpsTxt" placeholder="请输入帐号" style="width: 100%; height: 100%; text-indent: 15px;" id="cpsinp">
      </div>
    </template>
    <!-- 快递 -->
    <div class="addr" @click="showAddress" v-if="selected == '0' && !show_address">
    ...
    <template v-if="dyFormPopup">
      <van-popup v-model="dyFormPopup" position="top" class="mint-popup-4" closeOnClickModal="true" style="width: 100%; height: 100%;">
        ...
        <ul>
          <li v-for="(item, i) in addressData" :key="i" @click="selectAddress(item)">
            <div class="liia">
              ...
              <p class="address" style="" v-if="item.street">{{ item.province + " " + item.city + " " + item.district + "" + item.street }} {{ item.address }}</p>
              <p class="address" style="" v-else>{{ item.province + " " + item.city + " " + item.district }} {{ item.address }}</p>
            </div>
        ...
        <div>
          ...
          <van-cell-group :border="false" class="set-address">
            <van-cell :border="false" center is-link>
              ...
              <input slot="default" type="text" @click.stop="addressShow = true" v-model="addressName" readonly placeholder="请选择收货地址" onfocus="blur();" />
            </van-cell>
          ...
          <van-cell
            ...
            center
            is-link
            v-if="is_open_store_delivery && selected == 3"
