...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      (this.service_fee = []), (this.is_open_store_delivery = false);
      this.locationName = "";
    ...
    requestByAddress() {
      // 重新计算
      if (_root_tag == TAG_ACTION_BUY || _root_tag == "appointment_goods" || _root_tag == "yun_sign_goods" || _root_tag == "activity" || _root_tag == "starGroup" || _root_tag == "channel_buy") {
        this.order_data = "";
        this.getDataActionBuy(); // 直接购买
      } else if (_root_tag == "fromExchange") {
        this.order_data = "";
    ...
    assembleJson() {
      ...
      if (_root_tag == TAG_ACTION_BUY || _root_tag == "buyCar" || _root_tag == "appointment_goods" || _root_tag == "yun_sign_goods" || _root_tag == "activity") {
    ...
    selectCoupon(value, valObj, num) {
      ...
      // 重新计算
      if (_root_tag == TAG_ACTION_BUY || _root_tag == "appointment_goods" || _root_tag == "yun_sign_goods" || _root_tag == "activity" || _root_tag == "channel_buy") {
        this.getDataActionBuy(); // 直接购买
      } else if (_root_tag == "fromExchange") {
    ...
    changeConpon(num, data) {
      // console.log(num, data,"changeConpon");
    ...
    toGeolocation() {
      ...
      // this.$router.push(
      //   that.fun.getUrl("o2oLocation_loc", {
      //     tag: that.$route.query.tag,
      //     goodsId: that.$route.query.goodsId,
      //     optionsId: that.$route.query.optionsId,
      //     total: that.$route.query.total,
      //     id: that.diyform_id,
      //     form_data_id: that.form_data_id,
      //     cart_ids: that.$route.query.cart_ids,
      //     store_id: that.$route.query.store_id,
      //     delivery_status: that.delivery_status,
      //
      //     // 本页面的值
      //     address_data: that.form,
      //     create: that.create,
    ...
    initAddress() {
      this.form = this.$route.params.address_data;
      this.getStreet(this.district_id);
      this.create = this.$route.params.create;
      this.popAddAddress = true;

      this.form.longitude = this.$route.params.pointId ? this.$route.params.pointId.lng : "";
      this.form.latitude = this.$route.params.pointId ? this.$route.params.pointId.lat : "";
      this.addressName = this.form.province + " " + this.form.city + " " + this.form.district;
      this.locationName = "经度：" + this.form.longitude + "  纬度：" + this.form.latitude;
      // console.log(this.form);
    }
  },
  components: { cTitle, cDyPopup, deliveryMap },
  watch: {}
};
