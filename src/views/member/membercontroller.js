    ...
    */
    ...
    methods: {
            ...
            axios
                ...
                .then(response => {
                    if (response.data.result == 1) {
                        $http.post("member.qrcode.poster-record", { poster_id: that.poster_id, image: response.data.data.img_url }).then(response => {
                                if (response.result === 1) {
        ...
        getMemberInfo(data, order, plugins) {
            ...
            that.is_open_snatch_regiment =
                plugins.is_open.is_open_snatch_regiment == 1 ? true : false;
            that.is_hotel = plugins.is_open.is_open_hotel == 1 ? true : false;
