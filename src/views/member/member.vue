    ...
    <div id="member-all" v-show="template" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
      ...
      <section v-show="template == '03'">
        <div class="home" v-if="showDiy">
          <template v-for="(item, index) in showDiy">
            <component :key="index" :is="item.temp" :datas="item" :home="true" :index="index" :all="designer.length - 1"
                       :tag="345">
            </component>
          </template>
        ...
        <van-button
          type="primary"
          style="margin: 0.875rem auto; width: 90%;"
          size="large"
          color="#f15353"
          @click="outInfo($event)"
          v-if="myType == 5 || wechat_login_mode"
        >{{ $i18n.t("member.退出登录") }}
        </van-button>
        <!-- 版权图片 -->
      ...
      <div id="member" style="position: relative;" v-show="template != '03'">
          ...
          <div class="header-info">
            ...
            <div class="user-info">
              <div class="user-name" id="name-one">
                ...
                <span class="user-other" id="name-b" v-if="level_name && show_member_id != 1"
                >{{ $i18n.t("extension.会员") }}ID:{{ uid }}
                </span>
              </div>
          ...
          <van-collapse v-model="activeNames">
            ...
            >
              ...
              <div class="cart">
                <div
                  class="list1"
                  v-if="pluginsList.merchant && notShow.indexOf(item.url) < 0"
              ...
              <div class="cart">
                <div v-if="relation_switch" @click="openQrCode('block')">
                  <div class="list1">
                    <van-icon
                      slot="icon"
                      class="iconfont icon-member_code"
                      size="1.625rem"
                      color="#f15353"
                      style="margin-right: 0.375rem;"
                    ></van-icon>
                ...
                <router-link :to="fun.getUrl('myEvaluation', { selected: '1' })">
                  <div class="list1">
                    <van-icon
                      slot="icon"
                      class="iconfont icon-member_comment"
                      size="1.625rem"
                      color="#f15353"
                      style="margin-right: 0.375rem;"
                    ></van-icon>
                ...
                <router-link :to="fun.getUrl('myRelationship', { selected: '1' })">
                  <div class="list1">
                    <van-icon
                      slot="icon"
                      class="iconfont icon-member_relation"
                      size="1.625rem"
                      color="#f15353"
                      style="margin-right: 0.375rem;"
                    ></van-icon>
                ...
                <div>
                  <div
                    class="list1"
                    v-if="pluginsList.market && notShow.indexOf(item.url) < 0"
            ...
            <van-collapse-item name="3">
              ...
              <div class="cart">
                <div
                  class="list1"
                  v-if="pluginsList.asset_equity && notShow.indexOf(item.url) < 0"
        ...
        <div class="copyright">
          <!--v1.1.042-->
          <template v-if="fun.isApp()">
            <router-link style="color: #3b73ef;" :to="fun.getUrl('userAgreement', {}, { show: 'agreement' })"
            >用户协议
            </router-link
            >
            和
