...
export default {
  ...
  methods: {
    ...
    checkoutConsumePacket() {
      if (JSON.parse(window.localStorage.getItem("globalParameter")).consume_red_packet_status == 1) {
        // 判断消费红包开启再请求
        $http
          .post("plugin.consume-red-packet.Frontend.Modules.Packet.Controllers.has.index", {
          ...
          (response) => {
            if (response.result == 1) {
              if (response.data.redirect) {
                this.$dialog.confirm({ message: "支付成功" })
                  .then(() => {
          ...
          if (response.result === 1) {
            ...
            response.data.map((item) => {
              if (item.need_sign === 1 && item.contract_id && !flag) {
                flag = true;
                this.$dialog.alert({ message: `您的订单(订单号:${item.order_sn})在支付前需要签署电子合同` })
                  .then(() => {
  ...
  activated() {
    ...
    if (JSON.parse(window.localStorage.getItem("globalParameter")).is_open_shop_esign == "1") {
      this.getShopSign();
    } else {
      this.getMultipleOrderInfo();
    }
