    ...
    <div class="supplier">
      ...
      <div class="main">
        ...
        <div class="welcome">
          ...
          <van-cell-group>
            <van-field required v-model="form.qsnphone" label="企业电话" placeholder="请输入企业电话" />
            <van-cell required title="企业分类" :value="form.catename" is-link @click="toCategory" />
          </van-cell-group>
          ...
          <van-cell required value="企业LOGO" :border="false" />
          <div class="upload-container">
            <div class="upload-item">
              <van-uploader
          ...
          <van-cell required value="企业信息" :border="false" />
          <van-field
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    ...
    next(vm => {
      if (!(from.name === 'enterprise' || from.name === 'o2oLocation_loc')) {
        vm.getValidInfo()
      }
    })
  },
  methods: {
    ...
    getValidInfo() {
      ...
      return axios({
        ...
        // 轮播图
        this.form.topurl = res.data.data.topurl.length ? this.imgReview(res.data.data.topurl).map(item => item.url).join(',') : ''
        ...
        // 详情图片
        this.form.texturl =   res.data.data.texturl.length ? this.imgReview(res.data.data.texturl).map(item => item.url).join(',') : ''
