<template>
  <div id="income">
    <div class="supplier">
      <c-title :hide="false" text="发布商机"></c-title>
      ...
      <div class="main">
        ...
        <button :disabled="btnDisabled" class="btn" :class="{ normal: !btnDisabled }" @click="submitOrder">发布</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    submitOrder() {
      ...
      const url = 'https://tpkl.minpinyouxuan.com/index.php/api/v1/getshopsave'
      axios({
        method: 'POST',
        url,
        data: this.formData
    ...
    // 向后台请求微信支付signature
    async getSignature() {
      const url = 'https://tpkl.minpinyouxuan.com/index.php/api/v1/newshoppay'
      const formData = {
        ...
        wx.chooseWXPay({
          ...
          success: function (res) {
            ...
            if (res.errMsg == 'chooseWXPay:ok') {
              that.$router.push(that.fun.getUrl('shopRecord','',{ goods_id:that.formData.goods_id,}))
