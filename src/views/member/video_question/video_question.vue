<template>
  <div id="video_question">
    <van-nav-bar :title="info.name" fixed="true" left-arrow @click-left="onClickLeft" @click-right="onClickRight">
      <template #right>
        <i
          class="iconfont icon-fontclass-tongzhi"
          :class="[unread_message_count > 0 ? 'aaa' : '']"
          style="color: #adadad;"
        ></i>
      </template>
    </van-nav-bar>
    ...
    <div class="banner" v-if="info.carousel_list">
      <van-swipe class="my-swipe" :autoplay="3000" indicator-color="white" :show-indicators="false">
        <van-swipe-item v-for="(item, index) in info.carousel_list" :key="index"
          ><img :src="item.thumb" alt="" @click="tapSwiper(item.url)"
        /></van-swipe-item>
      </van-swipe>
    </div>
    <div class="grid">
    ...
    <div class="list" v-if="info.video_category_list">
      ...
      <div class="listBox">
        <div
          class="li"
          v-for="(data, ind) in info.video_category_list[tabind].video_list"
          :key="ind"
          @click="toWatch(data.url, data.id, data.title)"
        >
          <div class="img">
    ...
    <!-- 答题按钮 -->
    <div class="posBtn set-pc-styleLeft" @click="goAnswer" v-if="info.show_answer_button == 1"></div>
    <!-- 消息弹窗 -->
