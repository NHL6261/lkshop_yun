    ...
    <div id="content">
        ...
        <div class="goods" v-for="(order,i) in order_list" :key='i'>
          ...
          <ul class="info">
            <li>
              ...
              <span>{{$i18n.t('money')}}{{(Number(order.goods_price)/order.total).toFixed(2)}}</span>
            </li>
            <li class="standard">
      ...
      <ul class="order_info">
        ...
        <li v-if="orderType ==='robbing'">
          ...
          <span v-if="order_distributor">{{$i18n.t('money')}}{{order_distributor.dividend}}</span>
        </li>

        <h1 v-if="orderType !=='robbing'">实付款：<span><small>{{$i18n.t('money')}}</small>{{order_info.price}}</span></h1>
      </ul>
