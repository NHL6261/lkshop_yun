      ...
      <div class="goods" v-for="(item, inex) in info.goods" :key="item.id" @click.stop="showGoodPup(item.goods_id)">
        ...
        <div class="good-right">
          ...
          <div class="goods-purchased">
            <i class="iconfont icon-fontclass-qian"></i>利润￥{{ item.priceBonus }}
            <i class="iconfont icon-fontclass-kucun"></i>库存{{ item.stock }}件
          </div>
          <div class="good-buy">
            <div class="price">
            ...
            <div class="switch" @click.stop>
              <van-switch
                ...
                size="18px"
                active-color="#f14e4e"
                @change="ChangeSwitch(value, item.goods_id, inex)"
              />
            </div>
          </div>
        </div>
    ...
    ChangeSwitch(value, id, index) {
      $http
        .get(
          ...
          { group_id: this.$route.params.id, goods_id: id },
          ''
        )
        .then(response => {
          if (response.result === 1) {
    ...
    getPoster() {
      $http
        .get(
          ...
          { group_id: this.info.id, code_type: 1, get_type: 1, deliver_id: this.deliver_id },
          ''
        )
        .then(res => {
          if (res.result == 1) {
