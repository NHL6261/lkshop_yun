    ...
    <van-tabs v-model="activeName" sticky swipeable @change="handleClick">
      <van-tab :title="items" v-for="(items, ind) in titleArr" :key="ind">
        <ul class="content" v-if="!fun.isTextEmpty(dataList[ind].list)">
          <li
            v-for="(item, index) in dataList[ind].list"
            :key="index"
            @click.stop="gotoUrl('communityBuyDetails', { id: item.id })"
          >
            <div class="list">
              ...
              <div class="button-box">
                <div
                  class="btn"
                  v-if="item.group_status != 0"
                  @click.stop="gotoUrl('communityBuyOrder', { group_id: item.id })"
                >
                  订单
                </div>
    ...
    getPoster() {
      $http
        .get(
          ...
          { group_id: this.activeGroup.id, code_type: 1, get_type: 1, deliver_id: this.deliver_id },
          ''
        )
        .then(res => {
          if (res.result == 1) {
