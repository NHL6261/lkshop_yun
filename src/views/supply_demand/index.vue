<template>
  ...
  <div id="supply_demand" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
      ...
      <div id="tabs">
        <div @touchmove.prevent @mousewheel.prevent>
          <mt-navbar v-model="activeName" fixed="true" style="top:2.5rem;">
            <mt-tab-item id="first" @click.native="handleClick('0')">供应信息</mt-tab-item>
            <mt-tab-item id="second" @click.native="handleClick('1')">采购信息</mt-tab-item>
          </mt-navbar>
        </div>
        ...
        <mt-tab-container v-model="activeName" style="text-align:left;">
          <div class="choose" @touchmove.prevent @mousewheel.prevent>
            <div class="choose-head">
              ...
              <div class="choose-li" @click="chooseSort('2')" :class="sortSelectedWhich == 2 ? 'selected' : ''">
                <div class="van-ellipsis">
              ...
              <div class="choose-li" @click="chooseSort('5')" :class="sortSelectedWhich == 5 ? 'selected' : ''">
          ...
          <mt-tab-container-item id="first">
            ...
            <div class="list" v-else>
              <div class="list-li" v-for="(item, index) in recordsList" :key="index" @click="gotoDetail(item.id)">
                <div class="img" v-if="activeName == 'first'">
                  <img :src="item.product_pictures ? item.product_pictures[0] : ''" alt />
                </div>
                <div class="content">
                  <div class="title">
                    <div class="title-a">
                  ...
                  <div class="address">
                    <div>{{ item.province.n }} {{ item.city.n }} {{ item.name }}</div>
                  </div>
                  <div class="price">
                    <div class="price-word">单价</div>
                    <div class="price-num">{{ item.product_price }} {{ unit_price[item.unit_price] }}</div>
                  </div>
                  <div class="product">
                    <div class="product-word" style>产地：{{ item.product_origin }}</div>
                ...
                <div class="price-row">
                  <div class="price">
                    ...
                    <span style="color:#ff2c29;font-size:0.75rem"
                      >{{ item.purchasing_price }} {{ unit_price[item.unit_price] }}</span
                    >
                  </div>
                  <div class="product">
                    产地要求：{{
                      item.origin_requirement == 2 ? '不限' : item.origin_requirement == 0 ? '国内' : '国外'
                    }}
                  </div>
                </div>
                <div class="price-row">
                  <div class="price">我的位置：{{ item.province.n }} {{ item.city.n }}</div>
                  <div class="product">
                    状态要求：{{ item.product_type == 2 ? '不限' : product_state[item.product_type] }}
                  </div>
                </div>
    ...
    <!-- 底部发布按钮 -->
    <div class="fiexd-bottom" v-if="false">
      <div class="button" @click="gotoPublish">发布</div>
    </div>

    <div class="cursor" @click.stop="changeBtn">
      <!-- <img
      ...
      /> -->
      <div :class="[isOpenBtn ? 'rotateImg' : 'normalImg']" id="imgs">
      ...
      <div
        ...
        style="top:-2.625rem;left:0.5rem;"
        @click.stop="gotoPublish"
      >
        <i class="iconfont icon-supplier_release"></i>发布
      </div>
      <div class="btn btn_2" :class="[isOpenBtn ? 'active2' : '']" style="top:-2rem;left:-3.5rem;" @click.stop="my">
        <i class="iconfont icon-supplier_myrelease"></i>我的
      </div>
    </div>
    ...
    <div class="choose-content" v-show="sortContentShow">
            ...
            <div class="top-main">
              <div class="left">
                <div
                  ...
                  :class="pre_category_menus.parentid == item.id ? 'title-selected' : ''"
                >
                  <div class="title">{{ item.name }}</div>
                  <div class="icon">
                    <i style="font-size:12px" class="iconfont icon-advertise-next"></i>
                  </div>
                </div>
              </div>
              <!-- 二级分类样式 -->
              <div class="right" v-if="catlevel == 2">
                <div class="right-list">
                  <div
                    ...
                    :class="pre_category_menus.childid == item.id ? 'title-selected' : ''"
                  >
                    {{ item.name }}
                  </div>
                </div>
              ...
              <div class="right" v-if="catlevel == 3">
                <div v-for="(item, index) in second_list" :key="index">
                  ...
                  >
                    <div class="title" :class="pre_category_menus.thirdid == item1.id ? 'title-selected' : ''">
                      {{ item1.name }}
                    </div>
                  </div>
                </div>
        ...
        <div class="top">
          <div class="list">
            <div
              ...
              :class="supply_search.province_id == item.v ? 'title-selected' : ''"
            >
              {{ item.n }}
            </div>
          </div>
      ...
      <!-- 产地国家 -->
      <div v-if="activeName == 'first'" class="content-b" v-show="sortSelectedWhich == 3">
        <div class="top">
          <div class="list">
            <div
              class="title"
              ...
              :class="supply_search.product_origin == item ? 'title-selected' : ''"
            >
              {{ item }}
            </div>
          </div>
      ...
      <div class="content-d" v-else v-show="sortSelectedWhich == 4">
        <div class="top">
          <div class="list">
            <div
              class="title"
              :class="country_demand_word == '不限' ? 'title-selected' : ''"
            ...
            <div
              class="title"
              :class="country_demand_word == '国内' ? 'title-selected' : ''"
            ...
            <div
              class="title"
              :class="(country_demand_word == '国外') == 1 ? 'title-selected' : ''"
      ...
      <div class="content-d" v-show="sortSelectedWhich == 5">
        <div class="top">
          <div class="list">
            <div class="title" @click="changeState(2)" :class="product_type_word == '不限' ? 'title-selected' : ''">
              不限
            </div>
            <div
              class="title"
              ...
              :class="product_type_word == item ? 'title-selected' : ''"
            >
              {{ item }}
            </div>
          </div>
...
#supply_demand {
  ...
  .cursor {
    ...
    right: 0.5rem;
    color: #fff;
    background-image: linear-gradient(132deg, #fe7575 0%, #ff6333 100%), linear-gradient(#7f5e5e, #7f5e5e);
    ...
    .btn_2 {
      transform: scale(0.5) translateX(155%) translateY(85%);
    }
    .active2 {
      opacity: 1;
