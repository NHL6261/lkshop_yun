    ...
    <div class="popupClass2" @touchmove.stop :style="{ top: show ? '50%' : '-100%' }">
      <div class="popup-box" v-show="loadingImg">
        <div class="code" id="code_box">
          <div class="shop_top">
            <img :src="`${PosterData.shop_logo}?${Date.now()}`" alt="" class="shop_icon" />
            <div class="shop_name">{{ PosterData.shop_name }}</div>
          </div>
          <div class="bgBox">
            <img class="bg" :src="`${PosterData.cover}?${Date.now()}`" alt="" style="object-fit: cover;" />
            <div class="anchorInfo">
              <img :src="`${info.avatar}?${Date.now()}`" alt="" />
          ...
          <div class="posterBottom">
            <div class="l_info">
              ...
              <div class="share_meber">
                <img :src="`${PosterData.m_avatar}?${Date.now()}`" alt="" />
                <div class="m_nickname">{{ nameEllipsis }}</div>
                的分享
              </div>
            </div>
            <div class="r_info">
              <img :src="`${PosterData.qr_code}?${Date.now()}`" alt="" />
              <p>长按识别或者保存图片</p>
            </div>
          </div>
