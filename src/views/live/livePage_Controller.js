...
// var ctx = null;
var timer_1, timer_2, timer_hb, powerToast = null;  // 循环定时器
  ...
  methods: {
    ...
    quitRoom() {
      ...
      $http.post('plugin.room.frontend.live.quit-room', { room_id: this.RoomId }, "").then(response => {
        if (response.result === 1) { console.log(response); } else { Toast(response.msg); }
      })
        .catch(error => {
          console.log(error);
        });
    ...
    share(data) {
      ...
      wx.ready(function () {
        ...
        if (that.activatPlayType == 'livePlay') {
          ...
          _link = that.fun.getSiteRoot() + "/addons/yun_shop/" + "?menu#" + "/livePage?i=" + that.fun.getKeyByI() + "&shareType=memberPlayBack" + "&room_id=" + that.RoomId + "&backid=" + that.backPlayID + "&memberid=" + that.memberId;
        }
    ...
    setValue(e) {
      ...
      let value =
        ((e.clientX - that.$refs.slider[0].offsetLeft) /
          that.$refs.slider[0].offsetWidth) *
        (maxTime - minTime);
