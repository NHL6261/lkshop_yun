...
<template>
  <div id="livePage">
    ...
    <div
      class="active_goods_style"
      v-if="!songStop && !fun.isTextEmpty(activeLiveSetInfo.now_goods)"
      @click="gotoGoodDetail(activeLiveSetInfo.now_goods.goods_id)"
    >
      <p>当前讲解的商品</p>
      <div class="goods_main">
        <img :src="activeLiveSetInfo.now_goods ? activeLiveSetInfo.now_goods.image : ''" alt="" />
        <div class="price">
          {{ $i18n.t('money') }} {{ activeLiveSetInfo.now_goods ? activeLiveSetInfo.now_goods.price : '' }}
        </div>
      </div>
    ...
    <swiper :options="swiperOption" v-if="liveList.length > 0 && !swiperResize" ref="mySwiper">
      <swiper-slide v-for="(item, index) in liveList" :key="index">
        ...
        <div class="content" v-show="item.show">
          ...
          >
            ...
            <div class="anchorInfo" @click="anchorPopup">
              ...
              <div class="user">
                ...
                <p>
                  <template v-if="!fun.isTextEmpty(diy_set) ? isopenBtn(diy_set.location) : true"
                    >{{ dataInfo.address }}| </template
                  ><template v-if="!fun.isTextEmpty(diy_set) ? isopenBtn(diy_set.view) : true"
                    >{{ view_num }}观看</template
                  >
                </p>
              </div>
          ...
          <div
            class="directSeeding_content_center"
            v-if="(!fun.isTextEmpty(diy_set) ? isopenBtn(diy_set.banner) : true) && dataInfo.banner"
          >
          ...
          >
            ...
            <div style="display: flex; justify-content: space-between; position: relative;">
                    ...
                    <div class="text">
                      ...
                      <template v-if="!fun.isTextEmpty(item.text)">
                        <div class="chat_image">
                          ...
                          <div
                            v-if="!fun.isTextEmpty(diy_set) ? isopenBtn(diy_set.member_level) : true"
                            class="message_tag"
                            :class="[item.memberLevel > 10 ? '' : `message_tag_${item.memberLevel}`]"
                          >
                            {{ item.memberLevelName }}
                          </div>
                        </div>
                        <em class="name" :class="[item.memberLevel > 10 ? '' : `textcolor_${item.memberLevel}`]">{{
                          item.nickName
                        }}</em
                        >{{ item.text }}
                      </template>
              ...
              <div class="top_icon">
                <div
                  class="top_icons"
                  v-if="!fun.isTextEmpty(diy_set) ? isopenBtn(diy_set.report) : true"
                ...
                <div
                  class="top_icons"
                  v-if="!fun.isTextEmpty(diy_set) ? isopenBtn(diy_set.share) : true"
                  @click="shareMessage"
                  style="margin-bottom: 0.625rem;"
                >
                ...
                <div
                  class="top_icons"
                  v-if="!fun.isTextEmpty(diy_set) ? isopenBtn(diy_set.poster) : true"
                  @click="showPosterMethob"
                  style="margin-bottom: 0.625rem;"
                >
            ...
            <div class="bottomBox">
              <div style="display: flex;">
                <div
                  class="top_icons top_icons1"
                  @click="showGoodLift = true"
                  v-if="!fun.isTextEmpty(diy_set) ? isopenBtn(diy_set.shop_bag) : true"
                >
    ...
    <div
      class="popupClass_newPoster"
      @touchmove.stop
      v-if="!fun.isTextEmpty(PosterDatas)"
      :style="{ left: showPoster ? '50%' : '-150%' }"
    >
      <div class="popup-box" v-show="loadingImg">
        <div class="code" id="code_box">
          <div class="shop_top">
            <img :src="`${PosterDatas.shop_logo}?${Date.now()}`" alt="" class="shop_icon" />
            <div class="shop_name">{{ PosterDatas.shop_name }}</div>
          </div>
          <div class="bgBox">
            <img class="bg" :src="`${PosterDatas.cover}?${Date.now()}`" alt="" style="object-fit: cover;" />
            <div class="anchorInfo">
              <img :src="`${dataInfo.avatar}?${Date.now()}`" alt="" />
          ...
          <div class="posterBottom">
            <div class="l_info">
              ...
              <div class="share_meber">
                <img :src="`${PosterDatas.m_avatar}?${Date.now()}`" alt="" />
                <div class="m_nickname">{{ nameEllipsis }}</div>
                的分享
              </div>
            </div>
            <div class="r_info">
              <img :src="`${PosterDatas.qr_code}?${Date.now()}`" alt="" />
              <p>长按识别或者保存图片</p>
            </div>
          </div>
        </div>
      ...
      <div style="width: 19.375rem; margin: 0 auto; border-radius: 8px; z-index: 2147483647;" v-show="!loadingImg">
        <img
          style="width: 19.375rem; border-radius: 8px; z-index: 2147483647; left: 0;"
          id="thecanvas"
          @touchstart.stop="previewImage"
        />
      </div>
          ...
          <van-swipe-item v-for="(items, index) in giftList.list" :key="index">
            <div class="giftSwipe">
              <div
                ...
                :key="index"
                :class="{ active: isGiftId == item.id }"
                @click="selectLiveGift(item)"
              >
                <div class="img">
                  <img :src="item.icon_image" alt="" />
                </div>
                <div class="title">{{ item.name }}</div>
                <div class="other" v-if="giftList.member.credit1 && item.credit1_number && item.credit1_number != 0">
                  {{ giftList.member.credit1_name }}:{{ item.credit1_number }}
                </div>
                <div class="other" v-if="giftList.member.love && item.love_number && item.love_number != 0">
                  {{ giftList.member.love_name }}:{{ item.love_number }}
                </div>
                <div class="other" v-if="giftList.member.credit2 && item.credit2_number && item.credit2_number != 0">
                  {{ giftList.member.credit2_name }}:{{ item.credit2_number }}
                </div>
              </div>
            </div>
          </van-swipe-item>
        ...
        <div class="se-left">
          <van-radio-group v-model="radio">
            <van-radio name="1" checked-color="#ff3902" v-if="giftList.member.credit1 && rewardType.integral == '1'"
              >我当前{{ giftList.member.credit1_name }}数量{{
                giftList.member.credit1 ? giftList.member.credit1 : ''
              }}</van-radio
            >
            <van-radio name="2" checked-color="#ff3902" v-if="giftList.member.credit2 && rewardType.balance == '1'"
              >我当前{{ giftList.member.credit2_name
              }}{{ giftList.member.credit2 ? giftList.member.credit2 : '' }}</van-radio
            >
            <van-radio name="3" checked-color="#ff3902" v-if="giftList.member.love && rewardType.love == '1'"
              >我当前{{ giftList.member.love_name }}数量{{
                giftList.member.love ? giftList.member.love : ''
              }}</van-radio
            >
          </van-radio-group>
        </div>
          ...
          <div class="activeBox">
            ...
            <div style="max-height: 18rem; overflow-y: scroll;">
              <div class="activeList">
                <div
                  class="activeChild "
                  v-for="(item, indexs) in items.activityData.activityList"
                  :key="indexs"
                  :class="item.status == 3 ? 'failClass' : 'succedClass'"
                >
                ...
                <div
                  class="activeChild"
                  :class="items.activityData.pointBonusStatus == 3 ? 'failClass' : 'succedClass'"
                  v-if="items.activityData.pointBonus"
                >
                ...
                <div
                  class="activeChild"
                  :class="items.activityData.loveBonusStatus == 3 ? 'failClass' : 'succedClass'"
                  v-if="items.activityData.loveBonus"
                >
                ...
                <div
                  class="activeChild"
                  :class="items.activityData.balanceBonusStatus == 3 ? 'failClass' : 'succedClass'"
                  v-if="items.activityData.balanceBonus"
                >
                ...
                <div
                  class="activeChild"
                  :class="items.activityData.packetBonusStatus == 3 ? 'failClass' : 'succedClass'"
                  v-if="items.activityData.packetBonus"
                >
            ...
            <div
              class="receiveBtn"
              @click="activityDataBtn(index, items.activityData.activity_id)"
              :class="items.activityData.status == 2 ? 'failClass' : 'succedClass'"
            >
      ...
      <van-popup v-model="roomMemberContentArray[index].activityData.showactivity" round>
        <div class="popupClass5">
          <div class="activeBox">
            ...
            <div style="max-height: 18rem; overflow-y: scroll;">
              <div class="activeList">
                ...
                <div
                  class="activeChild"
                  :class="items.activityData.pointBonusStatus == 3 ? 'failClass' : 'succedClass'"
                  v-if="items.activityData.pointNum"
                >
                  <div class="price">
                    {{
                      items.activityData.pointSet == 1
                ...
                <div
                  class="activeChild"
                  :class="items.activityData.loveBonusStatus == 3 ? 'failClass' : 'succedClass'"
                  v-if="items.activityData.loveNum"
                >
                  <div class="price">
                    {{
                      items.activityData.loveSet == 1
                ...
                <div
                  class="activeChild"
                  :class="items.activityData.balanceBonusStatus == 3 ? 'failClass' : 'succedClass'"
                  v-if="items.activityData.balanceNum"
                >
                  <div class="price">
                    {{
                      items.activityData.balanceSet == 1
                ...
                <div
                  class="activeChild"
                  :class="items.activityData.packetBonusStatus == 3 ? 'failClass' : 'succedClass'"
                  v-if="items.activityData.packetNum"
                >
                  <div class="price">
                    {{
                      items.activityData.packeSet == 1
            ...
            <div
              class="receiveBtn"
              @click="roomMemberBtn(index, items.activityData.activity_id)"
              :class="items.activityData.status == 2 ? 'failClass' : 'succedClass'"
            >
              {{ items.activityData.status == 2 ? '已领取' : '立即领取' }}
            </div>
    ...
    <van-popup v-model="showWxpay" round closeable>
      <div class="popupClass7">
        ...
        <div class="paylist">
          <van-checkbox checked-color="#ff2c29" v-model="checked_wxPay" label-position="left" class="addStyle"
            ><i class="iconfont icon-card_weixin"></i> 微信支付</van-checkbox
          >
        </div>
    ...
    <van-popup
      ...
      round
      position="bottom"
      class="passWord_popup set-pc-styleLeft"
      :style="{ height: '30%' }"
