...
export default {
      ...
      contentList: [
        {
          ...
          addressInfo: {
            img: require('../../assets/newImg/index/addressIcon.png'),
            info: '海淀区 拣到互联技术有限公司'
          },
          labelInfo: [{ labelName: '科技' }, { labelName: '技术' }, { labelName: '互联网' }],
          photoInfo: [
            { url: require('../../assets/newImg/index/pic1.png') },
            { url: require('../../assets/newImg/index/pic1.png') },
            { url: require('../../assets/newImg/index/pic1.png') }
          ],
          receive: 30,
  ...
  methods: {
    ...
    judgeClassification() {
      this.page = 0
      if (this.$route.query.from === 'classificationentShop') {
        this.classification =
          JSON.parse(window.localStorage.getItem('classificationentShop')).length === 0
            ? [{ id: 1, name: '全部' }]
            : JSON.parse(window.localStorage.getItem('classificationentShop'))
    ...
    toCategory() {
      this.$router.push(this.fun.getUrl('classificationShop', '', { tag: 'listShop' }))
    },
