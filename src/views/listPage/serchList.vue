  ...
  <div class="content">
    ...
    <div class="serchTitle">
      ...
      <div
        ...
        @click="serchHandle"
      >搜索</div>
      <!-- <div class="classificationIcon" @click="toCategory">
        <div></div>
      </div> -->
    </div>
...
export default {
  ...
  methods: {
    ...
    judgeClassification() {
      this.page = 0
      if (this.$route.query.from === 'classificationentErprise') {
        this.classification =
          JSON.parse(window.localStorage.getItem('classificationentErprise')).length === 0
            ? [{ id: 1, name: '全部' }]
            : JSON.parse(window.localStorage.getItem('classificationentErprise'))
    ...
    toCategory() {
      this.$router.push(this.fun.getUrl('enterprise', '', { tag: 'listEnterprise' }))
    },
    toDetail(item) {
      if (item.qsntype === 1) {
        this.$router.push(
          this.fun.getUrl('detailEnterprise', '', { enterpriseId: item.id, receive_status: item.receive_status })
        )
      } else if (item.qsntype === 2) {
        this.$router.push(this.fun.getUrl('detailShop', '', { shopId: item.id, receive_status: item.receive_status }))
      } else if (item.qsntype === 3) {
        this.$router.push(
          this.fun.getUrl('detailPeople', '', { peopleId: item.id, receive_status: item.receive_status })
        )
      }
    },
