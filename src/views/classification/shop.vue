  ...
  <div class="content">
    <ul class="leftList">
      <li>
        <ul>
          <li
            v-for="(oneItem, oneIndex) in oneList"
            :key="oneItem.id"
          >
            <div
...
export default {
  ...
  methods: {
    ...
    checkoutFourList(twoLevel, threeLevel) {
      ...
      if (this.$route.query.tag === 'shopsInfo') {
        this.$router.push(this.fun.getUrl('shopsInfo', { category: threeLevel }))
      } else if (this.$route.query.tag === 'listShop') {
        let result = []
        result = result
        ...
        this.$router.push(this.fun.getUrl('listShop', '', { from: 'classificationentShop' }))
        window.localStorage.setItem('classificationentShop', JSON.stringify(result))
      }
    },
    // 获取分类数据
    getCateList() {
