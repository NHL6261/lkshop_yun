<template>
  <div id="fixed-price" v-if="loading">
    <div :style="{ display: activityStatus ? 'none' : 'block' }">
      ...
      >
        ...
        <div class="activity-info clearfix " :class="activityData.rule == 1 ? 'custom' : ''">
          <div class="left fl">
            <p class="goods-count" v-if="activityData.rule == 0">
              以下商品{{ activityData.pack_price }}元任选{{ activityData.goods_count }}件
            </p>
            ...
            <van-count-down
              :time="timeLength"
              format="DD天"
              :style="{ color: activityData.rule == 1 ? '#f87166' : '' }"
            />
          ...
          <span class="fr " :class="activityData.rule == 0 ? 'rules' : ''" @click="rulesDetailClick"
            >活动详情<i class="iconfont icon-advertise-next ico-arrow"></i
          ></span>
        </div>
      </div>
          ...
          <div class="goods-text">
            ...
            <div class="goods-price">
              ...
              <div class="select fr" @click.stop>
                <template
                  v-if="
                    (!goodsCartsTotal[item.id] && item.has_option != 1) ||
                      (goodsCartsTotal[item.id] && goodsCartsTotal[item.id].total == 0)
                  "
                >
                ...
                <template
                  v-if="goodsCartsTotal[item.id] && item.has_option != 1 && goodsCartsTotal[item.id].total != 0"
                >
        ...
        <div class="left-cart">
          ...
          <div class="cart-text">
            ...
            <p class="des" v-if="cartsNum - activityData.goods_count < 0 && cartsTotal != 0">
              再{{ activityData.goods_count - cartsNum }}买件,下单立享【{{ activityData.pack_price }}元任选{{
                activityData.goods_count
              }}件】
            </p>
            <p class="des" v-if="cartsNum >= activityData.goods_count" style="color: #f14e4e;">
              下单立享【{{ activityData.pack_price }}元任选{{ activityData.goods_count }}件】
            </p>
          </div>
        </div>
      ...
      <van-action-sheet v-model="rulesShow" title="规则详情">
        <div class="rules-content">
          ...
          <p class="rules-text">
            1、活动范围内，任选{{ activityData.goods_count }}件仅需{{ activityData.pack_price }}元
          </p>
    ...
    <div class="specs-win" style="height: 100%;">
      <template v-if="popupSpecs">
        <yz-specs
          ...
          v-on:closeSpecsPopup="close_yz_specs_popup"
          :carts="carts"
        >
        </yz-specs>
      </template>
    ...
    <!-- 购物车列表弹窗 -->
    <van-popup v-model="showCarList" round position="bottom" :style="{ height: '70%' }" closeable>
      <yz-cart-list
        :cardata="carList"
        class="carlist"
        v-on:blur="blurCart"
        v-on:plus="addGoods"
        v-on:minus="reduceGoods"
      >
        <!-- 底部购物车 -->
        <div class="go-shop" :class="[fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
          <div class="left-cart">
    ...
    <van-popup v-model="showgp" position="bottom" :style="{ height: '80%' }" closeable @closed="close">
      <yzGoodsPopup
        :showgp="showgp"
        :goodsInfo="goodinfo"
        v-on:clonegoodp="cloneGoodPopup"
        v-on:showgoodspecs="showgoodspecs"
      ></yzGoodsPopup>
    </van-popup>
              ...
              <div v-for="(item, index) in dispatch_types" :key="index" style="padding: 0.5rem 0;">
                ...
                <ul class="radio-list">
                  <li class="radio-item" v-for="(goods, i) in item.member_carts" :key="i">
                    <p>{{ goods.title }}</p>
                    <p v-if="goods.option_title" style="font-size: 12px; color: #666;">
