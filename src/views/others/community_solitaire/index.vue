    ...
    <div class="container">
                ...
                <img
                  :src="
                    item.has_one_member
                      ...
                      : require('../../../assets/images/img_default.png')
                  "
                />
              </div>
              <div class="text">
                <span
                >{{ item.has_one_member ? item.has_one_member.nickname : "" }}完成接龙，购买了{{
                    item.title_list[0]
                  }}</span
                >
              </div>
      ...
      <div class="countdown" v-if="info.end_time">
        <template v-if="info.state == 0">
          <template
            v-if="new Date().getTime() < Number(info.end_time) && new Date().getTime() > Number(info.start_time)"
          >
            距离活动结束还剩
      ...
      <div class="bottom-car set-pc-styleLeft">
        <div
          ...
          v-if="
            info.state == 0 &&
              new Date().getTime() < Number(info.end_time) &&
              new Date().getTime() > Number(info.start_time)
          "
        >
      ...
      <div class="float-nav set-pc-style">
        ...
        <router-link :to="fun.getUrl('orderlist', { status: 0 })" class="nav-child" tag="div"
        ><i class="iconfont icon-fontclass-shangpindingdan"></i>订单
        </router-link
        >
      </div>
    </div>
