  ...
  <div id="all">
    <template v-if="$store.state.isWatchDiypage == 1">
      ...
      <div :class="{ scroll: true }" v-show="!fullShow">
        ...
        <!--旧装修页面-->
        <div class="temp" v-if="$store.state.temp.item && !isDecorate">
          <!--旧装修的顶部导航-->
          <div
            class="search_nav"
          ...
          <div class="home" v-if="$store.state.temp.item.pageinfo">
            ...
            <template v-if="$store.state.temp.item.pageinfo.temp == 'topbar'">
              <div
                ...
                :class="{
                  'fe-floatico-right': $store.state.temp.item.pageinfo.params.floatstyle == 'right'
                }"
              >
                <a v-if="isMiniApp" @click="this.fun.gotoMiniAppCs">
                  <img :src="$store.state.temp.item.pageinfo.params.floatimg" style="height: 100%; width: 100%;"
                  /></a>
                <a v-if="!isMiniApp" :href="$store.state.temp.item.pageinfo.params.floathref"
                ><img :src="$store.state.temp.item.pageinfo.params.floatimg" style="height: 100%; width: 100%;"
                /></a>
              </div>
            </template>
            ...
            <div class="home" v-if="$store.state.temp.item.data && !isInDiy">
              <template v-for="(item, index) in $store.state.temp.item.data">
                <component :key="index"
                           ...
                           :tag="123"
                           :index="index"
                           :all="$store.state.temp.item.data.length"
                >
                </component>
              </template>
