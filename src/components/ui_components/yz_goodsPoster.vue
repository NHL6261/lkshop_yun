...
<template>
  <div id="goods-poster-main" :style="{ width: width }">
      ...
      <template v-if="isNewTemplate">
        ...
        >
          ...
          >
            ...
            <template v-else-if="item.type == 'title' || item.type == 'nickname'">{{ item.src }}</template>
    ...
    <div class="yz-goods-canvas" v-show="!fun.isTextEmpty(dataURL)">
      <img style="width: 100%;" id="goods_poster_thecanvas" />
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      needRotate: ["logo", "img", "other_img", "qr", "head"] //图片的type名称，需要转化base64
    };
  },
