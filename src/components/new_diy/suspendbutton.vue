    ...
    <template v-if="datas.list && datas.list.choose_style == 2">
      <div class="fixed-box2" :style="{ top: `${datas.over_top}px` }" v-if="datas.list.show_style == 1">
        <img
          ...
          :class="[isFold ? 'active' : '']"
          @click="toOpen"
          :src="isFold ? datas.list.fold_button || openImage : datas.list.unfold_button || openImage"
          class="imgs"
        />
        <div
          class="btn"
          ...
          :style="{
            top: `${-70 * (index + 1)}px`,
            left: `${
              datas.position_location == 'left'
                ? 10 + (datas.button_size - 50) / 2
                : box_width - 60 - (datas.button_size - 50) / 2
            }px`
          }"
        >
          <a :href="'tel:' + item.phone" v-if="item.is_minApp == 3">
      ...
      <div class="fixed-box2" :style="{ top: `${datas.over_top}px` }" v-if="datas.list.show_style == 2">
        ...
        <div
          ...
          style="top: -70px;"
          :style="btn_1_style"
          v-if="datas.list.list[0]"
        >
          <a :href="'tel:' + datas.list.list[0].phone" v-if="datas.list.list[0].is_minApp == 3">
            <img :src="datas.list.list[0].image || emptyImage" />
          </a>
          <img
            :src="datas.list.list[0].image || emptyImage"
            v-if="datas.list.list[0].is_minApp != 3"
            @click="toUrl(datas.list.list[0])"
          />
          <div
            style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; border-radius: 50%;"
            v-if="datas.list.list[0].is_minApp == 2"
            @click.stop
            v-html="datas.list.list[0].html"
          ></div>
        </div>
        <div class="btn" :class="[isFold ? 'active' : '']" :style="btn_3_style" v-if="datas.list.list[1]">
          ...
          <img
            :src="datas.list.list[1].image || emptyImage"
            v-if="datas.list.list[1].is_minApp != 3"
            @click="toUrl(datas.list.list[1])"
          />
          <div
            style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; border-radius: 50%;"
            v-if="datas.list.list[1].is_minApp == 2"
            @click.stop
            v-html="datas.list.list[1].html"
          ></div>
        </div>
        <div class="btn" :class="[isFold ? 'active' : '']" :style="btn_4_style" v-if="datas.list.list[2]">
          ...
          <img
            :src="datas.list.list[2].image || emptyImage"
            v-if="datas.list.list[2].is_minApp != 3"
            @click="toUrl(datas.list.list[2])"
          />
          <div
            style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; border-radius: 50%;"
            v-if="datas.list.list[2].is_minApp == 2"
            @click.stop
            v-html="datas.list.list[2].html"
          ></div>
        </div>
        <div
          class="btn"
          ...
          style="top: 70px;"
          :style="btn_2_style"
          v-if="datas.list.list[3]"
        >
          <a :href="'tel:' + datas.list.list[3].phone" v-if="datas.list.list[3].is_minApp == 3">
            <img :src="datas.list.list[3].image || emptyImage" />
          </a>
          <img
            :src="datas.list.list[3].image || emptyImage"
            v-if="datas.list.list[3].is_minApp != 3"
            @click="toUrl(datas.list.list[3])"
          />
          <div
            style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; border-radius: 50%;"
            v-if="datas.list.list[3].is_minApp == 2"
            @click.stop
            v-html="datas.list.list[3].html"
          ></div>
        </div>
      </div>
    </template>
      ...
      if (this.datas.list.choose_style == 1) {
        for (let j = 0; j < this.datas.list.list.length; j++) {
          ...
          this.datas.list.list[
            j
          ].html = `<wx-open-launch-weapp username="${this.datas.list.list[j].gh_id}" path="${this.datas.list.list[j].minApp_link}">${script.outerHTML}</wx-open-launch-weapp>`;
        }
      ...
      axios
        ...
        .then(response => {
          if (response.data.result == 1) {
            $http
              .post('member.qrcode.poster-record', { poster_id: that.poster_id, image: response.data.data.img_url })
              .then(response => {
                if (response.result === 1) {
