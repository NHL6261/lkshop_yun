          ...
          <ul class="weixin-sub-menu" v-show="selectedMenuIndex === i && btn.sub_button && btn.sub_button.length > 0">
            ...
            >
              <div
                class="menu-item-title"
                :style="{
                  lineHeight: tab_style.icon_set.icon_position == 'top' && tab_style.icon_set.icon_show && tab_style.text_set.text_show ? '25px' : '50px',
                  paddingTop: tab_style.icon_set.icon_position == 'top' ? '2px' : ''
                }"
              >
...
const defaultData = {
  page_info: {
    group_title: { title: "底部导航名称", type: "text", value: "默认" },
    button_item: {
      title: "",
      type: "bottomItem",
      choose: "2",
    ...
    tab_style: {
      ...
      type: "tab",
      value: {
        text_set: { text_show: true, text_color: "#333333", text_choose_color: "#ff4949" },
        icon_set: { icon_show: true, icon_position: "top", icon_color: "#333333", icon_choose_color: "#ff4949" },
    ...
    initMenuButton() {
      ...
      for (let i = 0; i < this.menu_button.length; i++) {
        ...
        for (let j = 0; j < this.menu_button[i].sub_button.length; j++) {
          ...
          script.type = "text/wxtag-template";
          script.text = `<div style="width: ${this.clientWidth / this.menu_button.length}px;height: 50px;"></div>`;
          this.menu_button[i].sub_button[
            j
          ].html = `<wx-open-launch-weapp username="${this.menu_button[i].sub_button[j].gh_id}" path="${this.menu_button[i].sub_button[j].minApp_link}">${script.outerHTML}</wx-open-launch-weapp>`;
        }
      }
