    ...
    <template v-if="list && list.length > 0">
      <div style="position: relative;" v-for="(item, index) in list" :key="index">
        ...
        <template v-if="item.hotArea && item.hotArea.length > 0">
          <div class="hotarea-rnd" v-for="(area,index) in item.hotArea" :style="{top: (area.top)*(clientWidth/400)+'px',left: area.left*(clientWidth/400)+'px',width: area.width*(clientWidth/400)+'px',height: area.height*(clientWidth/400)+'px',}" :key="index" @click="toAreaUrl(area)">
            <div style="position: absolute; width: 100%; height: 100%; top: 0; left: 0;"
                 v-if="area.is_minApp == 2" @click.stop v-html="area.html">
            </div>
          </div>
        </template>
      </div>
        ...
        for(let i = 0;i<this.datas.list.length; i++) {
          ...
          if(this.datas.list[i].hotArea) {
            for(let j=0; j<this.datas.list[i].hotArea.length; j++) {
              let script = document.createElement('script');
              script.type = 'text/wxtag-template';
              script.text = `<div style="width: ${this.datas.list[i].hotArea[j].width*(this.clientWidth/400)}px;height: ${this.datas.list[i].hotArea[j].height*(this.clientWidth/400)}px;"></div>`;
              this.datas.list[i].hotArea[j].html = `<wx-open-launch-weapp username="${this.datas.list[i].hotArea[j].gh_id}" path="${this.datas.list[i].hotArea[j].minApp_link}">${script.outerHTML}</wx-open-launch-weapp>`;
            }
          }
