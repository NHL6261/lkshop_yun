  ...
  <div>
    ...
    >
        ...
        <div class="store-img">
          ...
          <div class="store-category">
            <span
              ...
              :style="{ color: datas.params.catecolor,backgroundColor: datas.params.classcolor }"
            >{{ items.has_one_category.name }}</span>
            <span
              class="da store-category-name"
              :style="{ color: datas.params.catecolor,backgroundColor: datas.params.classcolor }"
              v-if="!items.has_one_category"
            >暂无分类</span>
          </div>
        </div>
        <!--new-->
        <div class="store-intro2">
          ...
          <div class="score-box">
            <div class="score-left">
              ...
              <span
                v-if="datas.params.showsale != '0'"
                style="margin-left: 0.5rem;"
              >月售{{ items.order_total }}单</span>
            </div>
            <div
              class="score-right"
              :style="{ color: datas.params.discolor }"
              @click.stop="goToAdress(items)"
              v-if="items.distance"
            >{{ items.distance }}{{ items.unit }}</div>
          </div>
          <div class="discount-box">
            <p
              class="discount-item"
              v-if="items.has_many_coupon && items.has_many_coupon.length > 0"
            >
              <span class="red" :style="{ color: datas.params.titlecolor }">优惠</span>
              <span
                style="margin-left: 0.5rem;"
                :style="{ color: datas.params.salecolor }"
                v-for="(coupon,index) in items.has_many_coupon"
                :key="index"
                v-if="index <=2"
              >{{ coupon.name }}</span>
            </p>
            <p
              class="discount-item"
              v-if="(datas.params.showscore != '0' || datas.params.showlove == 1 )&& (items.store_point.point_switch != 0 || items.store_love.award == 1)"
            >
              <span class="red" :style="{ color: datas.params.titlecolor }">赠送</span>
              <span
                style="margin-left: 0.5rem;"
                :style="{ color: datas.params.salecolor }"
                v-if="datas.params.showscore == '0' || items.store_point.point_switch == 0 ? false : true"
              >{{ items.store_point.point }}</span>
              <span
                style="margin-left: 0.5rem;"
                :style="{ color: datas.params.salecolor }"
                v-if="datas.params.showlove == 1 && items.store_love.award == 1 ? true : false"
              >{{ items.store_love.award_proportion }}</span>
            </p>
            <p
              class="discount-item"
              v-if="(datas.params.showscore != '0' || datas.params.showlove == 1 )&& (items.store_point.point_deduct_switch != 0 || items.store_love.deduction == 1)"
            >
              <span class="red" :style="{ color: datas.params.titlecolor }">抵扣</span>
              <span
                style="margin-left: 0.5rem;"
                :style="{ color: datas.params.salecolor }"
                v-if="datas.params.showlove == 1 && items.store_love.deduction == 1 ? true : false"
              >{{ items.store_love.deduction_proportion }}</span>
              <span
                style="margin-left: 0.5rem;"
                :style="{ color: datas.params.salecolor }"
                v-if="datas.params.showscore == 0 || items.store_point.point_deduct_switch == 0 ? false : true"
              >{{ items.store_point.max_point_deduct }}</span>
            </p>
          </div>
          <div class="dispatch-box" v-if="items.dispatchs">
            <span
              v-for="dispatch in items.dispatchs"
              :style="{ color: datas.params.shipcolor,borderColor: datas.params.shipcolor }"
              :key="dispatch"
            >{{dispatch}}</span>
          </div>
        </div>
      ...
      <div class="goods-box" v-if="items.recommend_goods && items.recommend_goods.length > 0">
        ...
        <div
          class="good-item"
          v-for="(good, i) in items.recommend_goods"
          :key="i"
