<template>
  <transition name="fade2">
    <div id="goods" v-show="showGood" :class="[fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
        ...
        <div style="-webkit-overflow-scrolling: touch;" id="main">
          ...
          <li class="middle-line"><small>{{$i18n.t('money')}}</small><b>3.2</b>&nbsp;/天</li> -->
            ...
            <div class="time" v-if="!isBegTime">
              ...
              <yd-countdown slot="right" :time="endTimeStr" class="bottom_time">
                <em style="padding:0.25rem 0.125rem;border-radius:0.125rem;">{%d1} <em>{%d2}</em> </em>天
            ...
            <div class="info-box">
              ...
              <van-col :span="24" id="titleBox">
                ...
                <van-col :span="5" class="coupon" v-if="isCup"
                  ><span @click="gotoCoupon" v-if="goodsInfo.availability == 1 ? true : false">
                    领券 <i class="fa fa-angle-right"></i></span
                ></van-col>
              </van-col>
            </div>
          </van-row>
          ...
          <van-row>
            <van-col :span="24" id="stockcontainer">
              ...
              <van-col :span="12" class="sale">
                {{ isRent ? '租赁次数' : '销量' }}:{{
                  parseInt(goodsInfo.show_sales) + parseInt(goodsInfo.virtual_sales)
                }}
              </van-col>
            </van-col>
          </van-row>
          ...
          >
            <span id="optiondiv" v-if="goodsCount > 0"
              >已选 <em style="padding-left:0.625rem;color:#333333;">{{ goodsCount + goodsInfo.sku }}</em></span
            >
          ...
          <div class="act-box hot-item" style="padding-bottom: 0.8rem;">
            ...
            <div class="hot-money">
              <div>
                ...
                <p>首付</p>
                <p>
                  <span class="hot-red">{{ staging_buy_car_goods.down_payment }}</span
                  >元
                </p>
              </div>
              <div>
                <p>月供</p>
                <p>
                  <span class="hot-red">{{ staging_buy_car_goods.monthly_supply }}</span
                  >元
                </p>
              </div>
              <div>
                <p>期数</p>
                <p>
                  <span class="hot-red">{{ staging_buy_car_goods.period_number }}</span
                  >期
                </p>
              </div>
            </div>
            ...
            <div class="tips" v-if="staging_buy_car_goods.attach_set == 0">
              <p class="gray" v-for="(item, i) in staging_buy_car_goods.attach_content" :key="i">
                <span class="icon-item">{{ item.name }}</span
                >{{ item.content }})
              </p>
            </div>

            <div class="gray" style="line-height: 1.4rem;" v-for="(item, j) in staging_buy_car_goods.titles" :key="j">
              {{ item }}
            </div>
          </div>
          ...
          <div class="act-box" v-if="help_center_enabled == 1">
            <div class="evaTop">
              ...
              <div class="evaTo" style="color:#ed0606;" @click="gotoHelp()">
                更多问题&nbsp;
                <i class="fa fa-angle-right" style="font-size:20px;"></i>
              </div>
            </div>
            <div class="question-box">
              <div v-for="(item, i) in questionTitle" v-if="i < 5" :key="i">
                <span @click="gotoHelp(i)" class="question">{{ item }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="act-box" v-if="package_deliver_enable == 1">
            ...
            <div>
              ...
              <div v-if="LocationError" style="padding:1rem 0;">
                定位失败,请手动<span style="color:#f15353" @click.stop="gotoCommentList">切换位置</span>
              </div>
              ...
              <div class="take-item" v-for="item in info" :key="item.id">
                ...
                <div class="right">
                  ...
                  <div class="third">
                    <div class="button">
                      <a @click.stop :href="'tel:' + item.deliver_mobile"
                        ><i class="iconfont icon-dianhua1"></i>联系商家</a
                      >
                    </div>
            ...
            <div id="tabs">
              <div ref="goodPage" id="goodPage">
                <div>
                  ...
                  <div class="like-list" v-if="this.fun.isTextEmpty(goodDetail.show_push) ? false : true">
                    <div class="box06">
                      <div class="img">
      ...
      <mt-popup v-model="popupSpecs" position="bottom" class="mint-popup-4 pcStyle_mint" closeOnClickModal="true">
      ...
      <mt-popup v-model="popupSpecs2" position="bottom" class="mint-popup-4 pcStyle_mint" closeOnClickModal="true">
        <div class="specifications">
          <div class="shopimg">
            <div id="chooser_img">
              <img :src="popThumb" />
      ...
      <yd-popup v-model="activityShow" position="bottom" height="60%">
        <yd-navbar title="活动" height="2.8125rem" fontsize="16px" color="#333" fixed class="pcStyle_ydT">
          <span slot="right">
        ...
        <div style="clear:both;margin-top:2.8125rem;" class="pcStyle_ydC">
          <yd-cell-group>
            <yd-cell-item v-for="(item, i) in activityArr" :key="i">
              ...
              <span slot="right" v-if="item.type == 'string'">{{ item.value }}</span>
      ...
      <yd-popup v-model="showParameter" position="bottom" height="60%">
        <yd-navbar title="商品参数" height="2.8125rem" fontsize="16px" color="#333" fixed class="pcStyle_ydT">
          <span slot="right">
      ...
      <div id="foot">
        <div class="addfav" :class="{ nocar: !isGoods }" type="info" v-if="!is_o2o">
          <template v-if="!favorite">
            ...
            <ul>
              <li>
                <i class="iconfont icon-goods_collect normal" @click="onFavorite(favorite)"></i>
              </li>
              <li>收藏</li>
            </ul>
          </template>
          <template v-if="favorite">
            ...
            <ul>
              <li>
                <i class="iconfont icon-collect-active active" @click="onFavorite(favorite)"></i>
              </li>
              <li>已收藏</li>
            </ul>
          </template>
        ...
        <div class="cart-btn">
          <a @click="show1 = true"><span class="white">立即咨询</span></a>
        </div>
        <div class="cart-btn">
