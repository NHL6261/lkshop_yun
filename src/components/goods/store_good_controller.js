...
export default {
  props: ["goods_id", "goods_type", "touchClose", "goods_info_store", "showDis", "show8", "is_room","wx_video_link"],
  data() {
    return {
      showBigImg: false,
  ...
  methods: {
        ...
        response => {
          if (response.result === 1) {
            ...
            if (this.goodsInfo.has_option == 1 && this.fun.isTextEmpty(optionsId)) {
              //有规格 但是还未选择
              this.popupSpecs = true;
            } else if (this.goodsInfo.has_option == 1 && !this.fun.isTextEmpty(optionsId)) {
              //有规格 但是已选择
    ...
    getVideoLink(){
      $http
        .get('plugin.wx-video-link.frontend.index.index', {id:this.$route.params.id}, "").then(response => {
          if (response.result === 1) {
    ...
    getCart(goods) {
      ...
      $http
        .get("plugin.store-cashier.frontend.shoppingCart.member-cart.index", { store_id: this.store_id }, "加载中")
        .then(response => {
          if (response.result == 1) {
