  ...
  methods: {
    ...
    postShow() {
      if (!this.fun.isTextEmpty(this.posterImg.base64Image)) {
        ...
        $http
          .get('goods.goods-poster.generate-goods-poster', { id: this.$route.params.id }, '...加载中')
          .then(response => {
            toastPoster.close()
            if (response.result == 1) {
    ...
    gotoCommentList() {
      this.$router.push(this.fun.getUrl('SelfCarry_info', { goodsId: goods_id, tag: 'goodsDetail' }, {}))
    },
    //获取时间-限时购
