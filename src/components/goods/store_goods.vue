<template>
  <transition name="fade2">
    <div id="goods" v-show="showGood" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
              ...
              <van-col :span="24" v-if="is_o2o & (!isBuy || (isBuy && !isBegTime))" style="position: relative; padding: 0 0.75rem;">
                ...
                <div v-if="o2oCount <= 0 && goods_info.has_option !== 1">
                  <div v-if="!isBuy || (isBuy && !isBegTime)">
                    <van-button type="danger" style="height: 2.25rem; padding: 0 15px; float: right; background-color: #f15353;" @click="addCart1(goodsInfo)" v-if="showDis">加入购物车 </van-button>
                  </div>
                </div>
              </van-col>
            </van-row>
      ...
      <div id="foot" v-if="!is_o2o">
        <div class="addfav" :class="{ nocar: !isGoods }" type="info" v-if="!is_o2o">
          <template v-if="!favorite">
            <ul>
              <li>
                <i class="fa fa-star normal normal" @click="onFavorite(favorite)"></i>
              </li>
              <li>收藏</li>
            </ul>
          </template>
          <template v-if="favorite">
            <ul>
              <li>
                <i class="fa fa-star active active" @click="onFavorite(favorite)"></i>
              </li>
              <li>已收藏</li>
            </ul>
          </template>
        ...
        <template v-if="!isMiniApp">
          <div class="cservice" v-if="!is_o2o">
            <a :href="cservice"
              >{{ isMiniApp }}
              <ul>
        ...
        <div :class="{ cart: true, cell: true, nocar: !isGoods || !isAddCart }" @click="addCart">
          加入购物车
        </div>
        <div class="buy cell" :class="{ nocar: !isGoods }" v-if="!is_o2o" @click="buyNow">
          立即购买
        </div>
      </div>
      <button class="good_speci" v-if="!is_o2o && isGoods && goodsInfo.has_option != 1 && goodsInfo.is_added == 1" @click="buyNow">
        <span id="optiondiv">{{ goodsCount > 0 ? "已添加" + goodsCount + goodsInfo.sku : "请选择商品数量" }}</span>
        <i class="fa fa-angle-right" style="margin-right: 0; float: right; line-height: 2.75rem; font-size: 1.625rem; color: #666;"></i>
      </button>
      ...
      <!-- 未添加商品 -->
      <div class="Go_settle" v-show="carts.length === 0 && showDis && !notLogin">
