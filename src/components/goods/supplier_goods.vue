    ...
    <div id="goods" v-show="showGood" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
        ...
        <div style="-webkit-overflow-scrolling: touch; min-height: 101vh;" id="main">
            ...
            <div class="time" v-if="isBuy && isBegTime">
              <span class="text">距开始还剩：</span>
               <van-count-down :time="fun.getTimeDifference(beginTime)" class="bottom_time" format="DD天HH时mm分ss秒" @finish="timeEnd">
                <template #default="timeData">
                  <em style="padding: 0.25rem 0.125rem; border-radius: 0.125rem;">{{ timeData.days <10 ?`0${timeData.days}` :timeData.days}}</em><span style="color: #fff;">天</span>
          ...
          <van-row>
            <div class="info-box">
              <div class="title-box">
                ...
                <h2>
                  <van-col :span="24" id="price" v-if="!isGoodsLove">
                    <div
                      class="show_price_box"
                      v-if="goodsInfo.goods_show && goodsInfo.goods_show.goods_detail_style == 1"
                    >
                    ...
                    <template v-if="!goodsInfo.goods_show || goodsInfo.goods_show.goods_detail_style != 1">
                      ...
                      <del
                        id="original-del"
                        v-if="parseInt(goodsInfo.market_price) && goodsInfo.market_price != goodsInfo.price"
                      >
              ...
              <div style="display: flex;">
                <div class="goods-dicount" v-if="goodsInfo.coupon && goodsInfo.coupon.coupon_method == 1">
                  优惠券{{ $i18n.t('money') }}:{{ goodsInfo.coupon.deduct }}
                </div>
                <div class="goods-dicount" v-if="goodsInfo.coupon && goodsInfo.coupon.coupon_method == 2">
                  折扣:{{ goodsInfo.coupon.discount }}折
                </div>
                <div class="pub-price" v-if="goodsInfo.team_sales_first">
                  推广赚{{ $i18n.t('money') }}{{ goodsInfo.team_sales_first }}
                </div>
              </div>
          ...
          <van-row v-if="goodsInfo.love_shoppin_gift">
            <van-col
              ...
              class="price_box"
              v-if="
                goodsInfo.love_shoppin_gift && (!goodsInfo.vip_level_status || goodsInfo.vip_level_status.status == 0)
              "
            >
          ...
          <div class="act-box">
            ...
            <ul
              class="act"
              @click="gotohuiyuan"
              v-if="goods_info.selfbuy_discount"
            ...
            >
              ...
              <i
                ...
                v-if="
                  goods_info.selfbuy_discount &&
                    goods_info.selfbuy_discount.link_to_level_page &&
                    goods_info.selfbuy_discount.link_to_level_page.show == 1
                "
              ></i>
            </ul>
          ...
          >
            <div style="flex: 1; display: flex;">
              <div class="user-img">
                <img
                  :src="supplierData.logo ? supplierData.logo : require('../../assets/images/loading.jpg')"
                  style="height: 100%;"
                />
              </div>
              <ul class="user-name">
              ...
              <div ref="goodPage" id="goodPage">
                <div>
                  ...
                  <div class="live_list_div section" v-if="is_room == 1 && !fun.isTextEmpty(recordsList)">
                    ...
                    <span class="live_more_btn" v-if="showMoreLive" @click.stop="getLiveList(true)">{{
                      $i18n.t('加载更多')
                    }}</span>
                    <span
                      style="display: inline-block; width: 100%; height: 25rem; background-color: #f5f5f5; padding: 1.5rem 0;"
                    ></span>
                  </div>
                </div>
              </div>
      ...
      <van-popup v-model="popupSpecs" :style="{overflow:'visible'}" position="bottom" class="mint-popup-4" closeOnClickModal="true">
        <div class="specifications">
          ...
          <van-button size="small" id="btsmall" type="danger" v-show="popStock >= 1" @click="submitAction"
            >确认</van-button
          >
      ...
      <van-popup class="poster-popup-class" v-model="posterShow" round
        ><yz-goodsposter :posterData="posterImg" :goodstype="true"></yz-goodsposter>
      </van-popup>
      ...
      <van-image-preview
        v-model="showSkuImages"
        :images="skuImages"
        @change="onChangeSkuImagesIndex"
        @close="onCloseSkuImages"
        :start-position="activeSkuIndex"
        :loop="false"
      >
        ...
        <template v-slot:cover>
          <div class="sku_price">
            ...
            <p v-if="goodsInfo.has_option == 1">
              {{ $i18n.t('money') }}{{ goodsInfo.has_many_options[activeSkuIndex].product_price }}
            </p>
